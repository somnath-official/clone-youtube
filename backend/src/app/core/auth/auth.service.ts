...
export class AuthService {
    ...
    async signIn( email: string, password: string ): Promise<{ access_token: string; refresh_token: string }> {
        ...
        const access_token = await this.jwtService.generateAccessToken({ id: user._id.toString(), sub: user.sub });
        const refresh_token = await this.jwtService.generateRefreshToken({ id: user._id.toString(), sub: user.sub });
    
        return { access_token, refresh_token };
    }

    async refreshTheAccessToken( refresh_token: string ): Promise<{ access_token: string }> {
        const { id, sub } = await this.jwtService.verifyRefreshToken(refresh_token)
        const access_token = await this.jwtService.generateAccessToken({ id, sub });
